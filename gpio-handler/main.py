...
class RampController:
    ...
    def init_mqtt(self) -> None:
        ...
        try:
            ...
            self.mqtt_engine.client.tls_insecure_set(
                True
            )  # Consider removing in production for secure connections
    ...
    def handle_mqtt(self, client: Any, userdata: Any, message: Any) -> None:
        ...
        try:
            ...
            if topic == "alpr/ramp/cmd":
                if data["value"] == 1:
                    if self.ramp_is_open or self.ignore_new_requests:
                        logging.debug(
                            "Ramp is currently open or new requests are being ignored, ignoring request."
                        )
                        return
                    self.open_ramp()
                    ...
                    self.ramp_open_timer = threading.Timer(
                        open_duration, self.send_ramp_close_command
                    )
                    self.ramp_open_timer.start()

                    ignore_duration = self.config["gate"]["ignore_duration"]
                    self.ramp_ignore_timer = threading.Timer(
                        ignore_duration, self.stop_ignoring_requests
                    )
            ...
            mqtt_message = {"error": "Exception in handle_mqtt", "message": str(e)}
    ...
    def open_ramp(self) -> None:
        ...
        try:
            ...
            mqtt_message = {"error": "Exception in open_ramp", "message": str(e)}
            self.mqtt_engine.publish(mqtt_message, "alpr/gpio-handler/error")
    ...
    def close_ramp(self) -> None:
        ...
        try:
            ...
            mqtt_message = {"error": "Exception in close_ramp", "message": str(e)}
            self.mqtt_engine.publish(mqtt_message, "alpr/gpio-handler/error")
...
if __name__ == "__main__":
    try:
        ...
        mqtt_message = {"error": "Exception in main", "message": str(e)}
